...
import tech.medo.plugin.MedoDeployedPlugin;
import tech.medo.plugin.bom.version.DependencyVersion;
import tech.medo.plugin.maven.MavenExec;
...
public class BomExtension {
	...
	public void library(String name, String version, Action<LibraryHandler> action) {
		...
		Library.LibraryVersion libraryVersion = new Library.LibraryVersion(DependencyVersion.parse(libraryHandler.version),
	...
	public void effectiveBomArtifact() {
		...
		this.project.getTasks().matching((task) -> task.getName().equals(MedoDeployedPlugin.GENERATE_POM_TASK_NAME))
				.all((task) -> {
	...
	public static class LibraryHandler {
		...
		public void group(String id, Action<GroupHandler> action) {
			...
			this.groups
					.add(new Library.Group(groupHandler.id, groupHandler.modules, groupHandler.plugins, groupHandler.imports));
		}
		...
		public void prohibit(String range, Action<ProhibitedVersionHandler> action) {
			...
			try {
				this.prohibitedVersions.add(new Library.ProhibitedVersion(VersionRange.createFromVersionSpec(range),
		...
		public class GroupHandler extends GroovyObjectSupport {
			...
			public void setModules(List<Object> modules) {
				this.modules = modules.stream()
						.map((input) -> (input instanceof Library.Module) ? (Library.Module) input : new Library.Module((String) input))
			...
			public class ModuleHandler {
				...
				public void exclude(Map<String, String> exclusion) {
					this.exclusions.add(new Library.Exclusion(exclusion.get("group"), exclusion.get("module")));
				}
		...
		public static class DependencyVersionsHandler {
			...
			public class ExtractFromHandler {
				...
				public void dependencyLock(String location) {
					DependencyVersionsHandler.this.dependencyVersions = new Library.DependencyLockDependencyVersions(location,
							DependencyVersionsHandler.this.libraryVersion);
				}

				public void dependencyConstraints(String location) {
					DependencyVersionsHandler.this.dependencyVersions = new Library.DependencyConstraintsDependencyVersions(
