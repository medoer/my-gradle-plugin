...
import tech.medo.plugin.MavenRepositoryPlugin;
			...
			if (dependencies != null) {
				for (Node dependency : findChildren(dependencies, "dependency")) {
					...
					Set<String> types = this.bom.getLibraries().stream()
							...
							.filter((group) -> group.getId().equals(groupId))
							.flatMap((group) -> group.getModules().stream())
							.filter((module) -> module.getName().equals(artifactId)).map(Library.Module::getType)
			...
			if (dependencies != null) {
				for (Node dependency : findChildren(dependencies, "dependency")) {
					...
					Set<String> classifiers = this.bom.getLibraries().stream()
							...
							.filter((group) -> group.getId().equals(groupId))
							.flatMap((group) -> group.getModules().stream())
							.filter((module) -> module.getName().equals(artifactId)).map(Library.Module::getClassifier)
							.filter(Objects::nonNull).collect(Collectors.toSet());
